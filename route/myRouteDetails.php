...
$route_CarID = $_SESSION['routeCarID'];
...
<body>
    <form action="submit.php" method="post">
        ...
        <div style="display: flex; justify-content: space-evenly;">
            ...
            $results = $conn->query($checkTravelingQuery);
            $rows = $results->fetch_assoc();
            $count = $rows['count'];

            $bookingss = "SELECT * FROM route  WHERE route_ID = '$route_ID'";
            $bookings = $conn->query($bookingss);
            $rowBookings = $bookings->fetch_assoc();

            if ($count == 0 && $rowBookings['route_Status'] == 'On-going') {
                echo '<input onclick="completed()" type="button" value="Completed">';
            }
            ?>
        </div>
        <br>
        ...
        if ($status != 'Cancelled') {
            $timeDeparted = strtotime($rowRouted['route_TimeDeparted']);
            $timeDepart = strtotime($rowRouted['route_Date'] . $rowRouted['route_Depart']);
            ...
            while ($rowPassenger = $passenger->fetch_assoc()) {
                ...
                if ($rowPassenger['booking_SeatPosition'] == 'Front Seat') {
                    ...
                    if ($FSStat == 'Arrived') {
                        ...
                        if ($timeDeparted <= $timeDepart) {
                            $FSRefund = $FSFare;
                        } elseif ($timeDeparted > $timeDepart && $timeDeparted <= ($timeDepart + 600)) {
                        ...
                        } elseif ($timeDifference > $tenMinutes) {
                    ...
                    if ($MSStat == 'Arrived') {
                        ...
                        if ($timeDeparted <= $timeDepart) {
                            ...
                            $MSRefund = $rowPassenger['booking_SeatRate'] * 0.4;
                        } elseif ($timeDifference > $tenMinutes) {
        ...
        <?php
        // echo $total ." ";
        // echo $totalRefundAdmin ." ";
        // echo $totalRefundPassenger . " ";
        // echo $timeDepart . " ";
        // echo $timeDeparted;
        ?>
        <br>
        ...
        <div class="confirmation" id="completed">
            ...
            <?php if ($timeDeparted <= $departureTime) : ?>
                <p>
            ...
            <?php elseif ($timeDeparted > $departureTime && $timeDeparted <= $departureTime + 600) : ?>
                <p>
            ...
            <?php endif; ?>
            <?php
            //     $booker = "SELECT * FROM user WHERE user_ID = '$user_ID'";
            // $accBal = $conn->query($user);
            // $rowBalance = $accBal->fetch_assoc();
            //     $balance = $rowBalance['user_AccBalance'];
            ...
            <div style="display: flex; justify-content: space-evenly; padding-bottom: 10px;">
                ...
                <input type="hidden" name="totalRefundAdmin" value="<?php echo $totalRefundAdmin ?>">
                <input type="hidden" name="totalRefundPassenger" value="<?php echo $totalRefundPassenger ?>">
                <!-- <input type="hidden" name="passengerID" value="<?php echo $route_ID ?>"> -->
                <input type="hidden" name="compRouteID" value="<?php echo $route_ID ?>">
                <input type="hidden" name="compRouteCarID" value="<?php echo $route_CarID ?>">
                <input onclick="closeDialog()" type="button" value="Cancel">
            </div>
        </div>
    </form>
